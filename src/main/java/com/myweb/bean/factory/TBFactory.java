package com.myweb.bean.factory;
...
public class TBFactory extends ShopBeanFactoryMB {
	...
	private String[] parentsT = new String[] { "\"nid\":.*?,",
			"\"category\":.*?,", "\"title\":.*?,", "\"raw_title\":.*?,",
			"\"pic_url\":.*?,", "\"detail_url\":.*?,", "\"view_price\":.*?,",
			"\"item_loc\":.*?,", "\"view_sales\":.*?,", "\"user_id\":.*?,",
			"\"nick\":.*?," };
	...
	protected ShopBean setBeanImf(HashMap<Integer, String> maps) {
		...
		for (Integer index : keys) {
			switch (index) {
			case shop_ID:
				bean.setShop_ID(maps.get(index));
				break;
			case price:
				bean.setPrice(maps.get(index));
				break;
			case img_Url:
				bean.setImg_Url(maps.get(index));
				break;
			case detail_Url:
				bean.setDetail_Url(maps.get(index));
				break;
			case title:
				bean.setTitle(maps.get(index));
				break;
			case raw_Title:
				bean.setRaw_Title(maps.get(index));
				break;
			case view_Sales:
				bean.setView_Sales(maps.get(index));
				break;
			case seller_ID:
				bean.setSeller_ID(maps.get(index));
				break;
			}
		}
