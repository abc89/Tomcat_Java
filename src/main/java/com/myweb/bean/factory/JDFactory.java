...
public class JDFactory extends ShopBeanFactoryMB {
	...
	// 商品信息分析 正则
	private String[] jdParents = new String[] { "data-sku=\".*?\"",
			"data-price=\".*?\"", "_blank\" title=\".*?\"",
			"flagsClk.*?</a>人评价", "href=\".*?\"", "data-lazy-img=\".*?\"" };
	/***
	 * 商品评论分析
	 */
	// 配查找内容
	...
	protected ShopBean setBeanImf(HashMap<Integer, String> maps) {
		...
		for (Integer index : keys) {
			...
			case img_url:
				...
				break;
			case deteil_url:
				bean.setDeteil_url(maps.get(index));
				break;
			case title:
				bean.setTitle(maps.get(index));
				break;
			case commentNum:
				bean.setCommentNum(maps.get(index));
			}
		}
