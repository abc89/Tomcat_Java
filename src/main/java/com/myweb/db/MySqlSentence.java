...
public class MySqlSentence {
	/*
	 * 注意空格!!!!!!!!!!!!!! 两两直接要加空格 此处JDBC驱动 表项 统一不可加单引号“'”？ 可加可不加分好“;” 表项
	 * [id]加方括号错误
	 */
	/**
	 * 
	 ...
	 * @return
	 */
	public static String getInsert(boolean auto, String tableName,
			String[] keys, Object... params) {
	 ...
	 * @return
	 */
	public static String getDeleteTable(String tableName, String key,
			Object value) {
	 ...
	 * @return
	 */
	public static String getUpdateTable(String tableName, String whereKey,
			String keyvalue, String upKey, String upValue) {
		return "update " + tableName + " set " + upKey + "=" + upValue
				+ " where " + whereKey + "='" + upKey + "'";
	}
	 ...
	 * @return
	 */
	public static String getUpdateTable(String tableName, String[] keys,
			Object... params) {
		...
		for (int i = 1; i < keys.length; i++) {
			...
			if (i != (keys.length - 1)) {
				...
				if (params[i] instanceof String || params[i] instanceof Integer) {
					updateSentence = updateSentence + params[i] + "' where "
							+ key + "='" + keyValue + "'";
				} else {
					updateSentence = updateSentence + null + "' where " + key
							+ "='" + keyValue + "'";
				}
			}
		}
